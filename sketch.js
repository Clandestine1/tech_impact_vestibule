...
function setup() {
  ...
  for (var k = 0; k < 2; k++) {
    rotation[k] = new RotateFlower(0, 0, 80, 100, 40);
  }
}
...
function draw() {
  ...
  for (var q = 0; q < rotation.length; q++) {
    push();
    translate(width*0.2, height*0.5); //need to change later to adjust location on canvas
    rotate(frameCount / 200.0);
    rotation[q].star(0, 0, 80, 100, 40);
    pop();
  }
